...
export class EventCheckinsService {
  ...
  findByEvent(eventId: string) { return this.prisma.eventCheckin.findMany({ where: { eventId }, orderBy: { checkedInAt: 'desc' } }); }
  findByUser(userId: string) { return this.prisma.eventCheckin.findMany({ where: { userId } }); }
